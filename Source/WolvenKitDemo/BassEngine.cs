...
namespace Sample_BASS
{
    public class BassEngine : IWaveformPlayer, ISpectrumPlayer
    {
        ...
        {
            Application.Current.Dispatcher.BeginInvoke(new Action(() => Stop()));
        }

        private void RepeatCallback(int handle, int channel, int data, IntPtr user)
        {
            Application.Current.Dispatcher.BeginInvoke(new Action(() => ChannelPosition = SelectionBegin.TotalSeconds));
        }
        #endregion
